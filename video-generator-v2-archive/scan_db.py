...
def processScan(captureId, dry_run=False):
        ...
        try:
            ...
            if currProcess:
                if currProcess['type'] == 'Scan':
                    ...
                    moveNext = processScan(captureId, dry_run)
                    if moveNext:
                        # finishRemoteProcess(processId)
                        # sendPlayerVideo(processId)
            ...
            scanPath = "{0}/{1}".format(takesDir, captureId)
            errorFile = "{0}/errors.txt".format(scanPath)
