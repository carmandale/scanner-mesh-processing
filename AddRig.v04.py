...
def find_front_and_back_vertices_points(obj, body_part:BodyPart, debug=False):
    ...
    point_front = Point(vertices_coords[0].x, vertices_coords[0].y, vertices_coords[0].z)

    # The point with the highest y-coordinate
    point_back = Point(vertices_coords[-1].x, vertices_coords[-1].y, vertices_coords[-1].z)
...
def get_feet_points(scan_obj, left_ankle, right_ankle, desired_height=0.016):
    ...
    # Get all the vertices that represent the right foot, sorted by the y-coordinate
    ...
    # The right toe tip is the vertex with the lowest y-coordinate
    ...
    if left_ankle is not None and right_ankle is not None:
        ...
        # Return all the calculated points
        return left_toe_tip_point, right_toe_tip_point, left_foot_heel_point, right_foot_heel_point, left_toe_base, right_toe_base

    # If we do not have information about the ankle points, return None for the toe base points
    return left_toe_tip_point, right_toe_tip_point, left_foot_heel_point, right_foot_heel_point, None, None
...
def update_body_parts_data(scan_obj, body_parts:BodyPart):
    ...
    for body_part in body_parts:
        ...
        # finding shoulder parts, changing the name to arms (since the parts are actually the arms head) and appending it to arms_parts
        ...
        if any(word in body_part.name for word in ("left_pinky", "left_index", "left_thumb")):
            left_fingers.append(body_part)

        if any(word in body_part.name for word in ("right_pinky", "right_index", "right_thumb")):
            right_fingers.append(body_part)

        if "hip" in body_part.name:
            hip_parts.append(body_part)

        if "left_ankle" == body_part.name:
            left_ankle = body_part

        if "right_ankle" == body_part.name:
            right_ankle = body_part
    ...
    shoulder_left = shoulder_points[1]
    shoulder_mid = shoulder_points[2]
    shoulder_right = shoulder_points[3]

    # Calculate mouth mid points
    mouth_mid = find_middle_point_using_locations([part.point_mid for part in mouth_parts])
    mouth_shoulder_mid = find_middle_point_using_locations((shoulder_mid, mouth_mid))
    mouth_front_mid = find_middle_point_using_locations([part.point_front for part in mouth_parts])
    mouth_back_mid = find_middle_point_using_locations([part.point_back for part in mouth_parts])

    # Calculate fingers mid points
    left_fingers_mid = find_middle_point([part.point_mid for part in left_fingers])
    right_fingers_mid = find_middle_point([part.point_mid for part in right_fingers])
    
    scan_max_z = find_object_max_z(scan_obj)
    head_top_mid = Point(mouth_mid.x, mouth_mid.y, scan_max_z)    

    # Compute spine points
    hip_mid = find_middle_point_using_locations([part.point_mid for part in hip_parts])
    spine_division_points = divide_space_between_two_points(point_start=hip_mid, point_end=shoulder_mid, n_parts=5)

    # Compute feet points
    feet_points_height = 0.016
    left_toe_tip, right_toe_tip, left_foot_heel, right_foot_heel, left_toe_base, right_toe_base = get_feet_points(scan_obj, left_ankle, right_ankle, desired_height=feet_points_height)

    # Compute average point between shoulder_mid, spines and hip
    # This is to have a centered straight line of points
    spine_complete_points = [shoulder_mid, hip_mid] + spine_division_points
    spine_mid = find_middle_point(spine_complete_points)

    # Set shoulder_mid, hip_mid and spine_points x, y values to the spine_mid x, y values
    spine_division_points = [Point(spine_mid.x, spine_mid.y, point.z) for point in spine_division_points]
    shoulder_mid = Point(spine_mid.x, spine_mid.y, shoulder_mid.z)
    hip_mid = Point(spine_mid.x, spine_mid.y, hip_mid.z)

    # Add additional body parts
    point_zero = Point(0,0,0)
    body_parts.append(BodyPart("left_shoulder", shoulder_left, point_zero, point_zero))
    body_parts.append(BodyPart("mid_shoulder", shoulder_mid, point_zero, point_zero))
    body_parts.append(BodyPart("right_shoulder", shoulder_right, point_zero, point_zero))

    body_parts.append(BodyPart("mouth_mid", mouth_mid, point_zero, point_zero))
    body_parts.append(BodyPart("mouth_shoulder_mid", mouth_shoulder_mid, point_zero, point_zero))
    body_parts.append(BodyPart("mouth_front_mid", Point(mouth_mid.x, mouth_front_mid.y, mouth_mid.z), point_zero, point_zero))
    body_parts.append(BodyPart("mouth_back_mid", Point(mouth_mid.x, mouth_back_mid.y, mouth_mid.z), point_zero, point_zero))

    body_parts.append(BodyPart("left_fingers_mid", left_fingers_mid, point_zero, point_zero))
    body_parts.append(BodyPart("right_fingers_mid", right_fingers_mid, point_zero, point_zero))

    body_parts.append(BodyPart("head_top_mid", head_top_mid, point_zero, point_zero))

    body_parts.append(BodyPart("hip_mid", hip_mid, point_zero, point_zero))

    body_parts.append(BodyPart("left_toe_tip", left_toe_tip, point_zero, point_zero))
    body_parts.append(BodyPart("right_toe_tip", right_toe_tip, point_zero, point_zero))
    body_parts.append(BodyPart("left_foot_heel", left_foot_heel, point_zero, point_zero))
    body_parts.append(BodyPart("right_foot_heel", right_foot_heel, point_zero, point_zero))

    if left_toe_base is not None:
        body_parts.append(BodyPart("left_toe_base", Point(left_toe_base.x, left_toe_base.y, feet_points_height), point_zero, point_zero))

    if right_toe_base is not None:
        body_parts.append(BodyPart("right_toe_base", Point(right_toe_base.x, right_toe_base.y, feet_points_height), point_zero, point_zero))

    # Add spine points
    for i, spine_location in enumerate(spine_division_points):
        if i == 0 or i == len(spine_division_points) - 1:
            continue
        body_parts.append(BodyPart(f"spine{i-1}_mid", Point(*spine_location), point_zero, point_zero))
...
def snap_bones(armature, body_parts):
    ...
    for bone in edit_bones:
        ...
        if bone.name == "mixamorig:RightUpLeg":
            ...
            bone.tail = body_parts_map["right_knee"].point_mid.to_vector()

        if bone.name == "mixamorig:LeftLeg":
            bone.head = body_parts_map["left_knee"].point_mid.to_vector()
            bone.tail = body_parts_map["left_ankle"].point_mid.to_vector()

        if bone.name == "mixamorig:RightLeg":
            bone.head = body_parts_map["right_knee"].point_mid.to_vector()
...
def main():
    ...
    armature_filepath = os.path.join(software_path, "skeleton_template_v04.blend")
    ...
    # CREATE BODY PARTS POINTS
    print_decorated("CREATE BODY PARTS POINTS")
    body_parts = get_body_parts_from_keypoints(results_filepath)
    update_body_parts_data(scan_obj, body_parts)

    keypoints = []
    for name, point_mid, point_front, point_back in (body_part.to_tuple() for body_part in body_parts):
    ...
    snap_bones(armature, body_parts)
