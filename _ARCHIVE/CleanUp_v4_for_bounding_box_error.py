...
def import_usda_model(path, scan_ID, import_path):
    ...
    if Geom_obj is None:
        print_enhanced("import_usda_model: Geom object not found", text_color="red", label="ERROR", label_color="red")
        return None
    
    if not Geom_obj.children:
        print_enhanced("import_usda_model: No children in Geom object", text_color="red", label="ERROR", label_color="red")
        return None
...
def find_vertices_outside_of_bounds_and_delete_them(obj, min_bounds=(-0.75, -0.75, -2), max_bounds=(0.75, 0.75, 4)):
    # Ensure the object is a mesh
    if obj is None:
        print_enhanced("bisect_mesh failed | scan_obj = None", text_color="red", label="ERROR", label_color="red")
        return

    print_enhanced(f"find_vertices_outside_of_bounds_and_delete_them | obj= {obj} | min_bounds: {min_bounds} | max_bounds: {max_bounds}", label="INFO", label_color="yellow")
    ...
    # Loop through vertices and select the ones within the bounds
    for v in bm.verts:
        if not all(min_bound <= coord <= max_bound for min_bound, coord, max_bound in zip(min_bounds, v.co, max_bounds)):
...
def main():
    ...
    find_vertices_outside_of_bounds_and_delete_them(scan_obj)
