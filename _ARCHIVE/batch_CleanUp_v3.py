...
def import_usda_model(path, scan_ID, import_path):
    ...
    # get the first children of the object 'Geom' and set it's name to 'g0'
    Geom_obj = bpy.data.objects.get('Geom')
    if Geom_obj is None:
        return print_enhanced("import_usda_model: Geom object not found", text_color="red", label="ERROR", label_color="red")
    
    if not Geom_obj.children:
        return print_enhanced("import_usda_model: No children in Geom object", text_color="red", label="ERROR", label_color="red")
...
def add_floor_v1(obj, floor_height, padding):
    if obj is None:
        print_enhanced("add_floor failed | obj = None", text_color="red", label="ERROR", label_color="red")
        return
...
def extract_floor_v3(scan_obj, extract_height=0.011):
    if scan_obj is None:
...
def extract_floor_v2(scan_obj, threshold=0.0001):
    print_enhanced(f"extract_floor | threshold: {threshold}", label="INFO", label_color="yellow")
    ...
    lowest_vertices_1 = [vert for vert in obj_data.vertices if vert.co.z < -4]
    if not lowest_vertices_1:
        print_enhanced("extract_floor failed | lowest_vertices_1 is None", text_color="red", label="ERROR", label_color="red")
        return

    for vertex in lowest_vertices_1:
        vertex.select = True

    bpy.ops.object.mode_set(mode='EDIT')

    print_enhanced(f"Selecting cube", label="INFO", label_color="yellow")
    bpy.ops.mesh.select_linked(delimit=set())

    print_enhanced(f"Extracting cube", label="INFO", label_color="yellow")
...
def extract_floor_v1(scan_obj, threshold, extrude_distance, recalculate_inside=True):
    if scan_obj is None:
        print_enhanced("extract_floor_v1 failed | scan_obj = None", text_color="red", label="ERROR", label_color="red")
        return
    
    print_enhanced(f"extract_floor_v1 | threshold: {threshold} | extrude_distance: {extrude_distance}", label="INFO", label_color="yellow")
    ...
    bpy.ops.mesh.normals_make_consistent(inside=recalculate_inside)
...
def separate_lower_legs_v1(scan_obj_copy):
    if scan_obj_copy is None or scan_obj_copy.type != 'MESH':
        print_enhanced("separate_lower_legs_v1 failed | obj = None or obj.type != 'MESH'", text_color="red", label="ERROR", label_color="red")
        return
    
    print_enhanced(f"separate_lower_legs_v1 | obj: {scan_obj_copy.name}", label="INFO", label_color="yellow")
    ...
    # Find vertices based on height
    vertices_to_remove = [v for v in bm.verts if v.co.z > height_threshold]

    # Remove the undesired vertices from the bmesh
    for v in vertices_to_remove:
        bm.verts.remove(v)

    # Update the mesh data with the modified bmesh
    bm.to_mesh(scan_obj_copy.data)
    bm.free()
...
def separate_lower_legs_v2(scan_obj_copy):
    ...
    print_enhanced(f"separate_lower_legs_v2 | obj: {scan_obj_copy.name}", label="INFO", label_color="yellow")
...
def main(scan_ID):
    ...
    extract_floor_v3(scan_obj, extract_height=floor_height)
