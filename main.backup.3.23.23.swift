...
let grooveMesherGroup = DispatchGroup()
...
struct GrooveMesher: ParsableCommand {
    ...
    @Flag(help: "Create a preview model.")
    private var createPreview: Bool = false

    @Flag(help: "Get the person bounding box.")
    private var getBoundingBox: Bool = false

    @Flag(help: "Create the final model using the person bounding box.")
    private var createFinalModel: Bool = false
                ...
                for try await output in session.outputs {
                    switch output {
                        ...
                        case .requestComplete(let request, let result):
                                        self.handleRequestComplete(request: request, result: result, session: session)
...
//                print("Using requests: \(String(describing: requests))")
                ...
                grooveMesherGroup.enter()
                if createPreview || (!createPreview && !createFinalModel) {
                        let (previewRequest, previewModelURL) = makeRequestForPreview()
                ...
                grooveMesherGroup.notify(queue: .main) {
                        if getBoundingBox || (!createPreview && !createFinalModel) {
                            let personBoundingBox = self.findBoundingBoxOfScannedPerson(previewModelURL: previewModelURL)
                            print("🟢 Person bounding box: \(personBoundingBox)")
                        }

                        if createFinalModel || (!createPreview && !createFinalModel) {
                            let finalRequest = self.makeRequestFromArguments(customBoundingBox: personBoundingBox)
...
//                                                               max: SIMD3<Float>((maximumX ?? 0.74), (maximumY ?? 1.2), (maximumZ ?? 0.74)))
    ...
    func findBoundingBoxOfScannedPerson(previewModelURL: URL) -> BoundingBox {
        ...
        // ...
        let floorYCoordinate = extractFloorYCoordinate()
        let calculatedBoundingBox = BoundingBox(min: SIMD3<Float>((minimumX ?? -0.74), (minimumY ?? (floorYCoordinate + (customFloorPad ?? 0.03))), (minimumZ ?? -0.74)),
                                                max: SIMD3<Float>((maximumX ?? 0.74), (maximumY ?? 1.2), (maximumZ ?? 0.74)))
    ...
    private func makeRequestForPreview() -> (PhotogrammetrySession.Request, URL) {
        ...
        case .modelFile(let url):
            ...
            if url.deletingPathExtension().lastPathComponent == "preview" {
                DispatchQueue.main.async {
                    // Pass the URL of the preview model to the group.notify block
                    grooveMesherGroup.notify(queue: .main) {
                        // Find the bounding box of the scanned person
                        let personBoundingBox = self.findBoundingBoxOfScannedPerson(previewModelURL: url)
